...
model SaltTwoTanks
	...
	parameter Real SM = 3.8 "Solar multiple";
	...
	parameter Real C = 1046.460400794 "Concentration ratio,relacion area del campo sobre el area del recibidor";
	...
	parameter Real ar_rec = 18.4/14 "Height to diameter aspect ratio of receiver aperture";
	...
	// Storage
	parameter Real t_storage(unit = "h") = 20 "Hours of storage";
	...
	parameter SI.Temperature T_hot_aux_set = CV.from_degC(565) "Hot tank auxiliary heater set-point temperature";
	...
	parameter Real tank_ar = 50.4/14 "storage aspect ratio";
	...
	parameter Real hot_tnk_full_lb = 70 "Hot tank full trigger lower bound";
	parameter Real hot_tnk_full_ub = 80 "Hot tank full trigger upper bound";
	...
	SolarTherm.Models.Sources.DataTable.DataTable data(
		...
		t_zone = t_zone,
		year = year,
		file = wea_file) annotation(Placement(visible = true, transformation(extent = {{-132, -56}, {-102, -28}}, rotation = 0)));

	//DNI_input
	Modelica.Blocks.Sources.RealExpression DNI_input(
		y = data.DNI) annotation(Placement(transformation(extent = {{-140, 60}, {-120, 80}})));

	//Tamb_input
	Modelica.Blocks.Sources.RealExpression Tamb_input(
		y = data.Tdry) annotation(Placement(transformation(extent = {{140, 70}, {120, 90}})));
	...
	Modelica.Blocks.Sources.RealExpression Pres_input(
		y = data.Pres) annotation(Placement(transformation(extent = {{76, 18}, {56, 38}})));

	//parasitic inputs
	Modelica.Blocks.Sources.RealExpression parasities_input(
		y = heliostatsField.W_loss + pumpHot.W_loss + pumpCold.W_loss + tankHot.W_loss + tankCold.W_loss) 
		annotation(Placement(transformation(extent = {{-13, -10}, {13, 10}}, rotation = -90, origin = {109, 60})));

	// Or block for defocusing
	Modelica.Blocks.Logical.Or or1 annotation(Placement(transformation(extent = {{-102, 4}, {-94, 12}})));
	...
	SolarTherm.Models.CSP.CRS.Receivers.ReceiverSimple receiver(
		...
		t_tb = t_tb_rec,
		D_tb = D_tb_rec,
		ab = ab_rec) annotation(Placement(transformation(extent = {{-46, 4}, {-10, 40}})));

	// Three-way Valve
	SolarTherm.Models.Fluid.Valves.ValveControl Valve1(
		redeclare package Medium = Medium)
	...
	SolarTherm.Models.Storage.Tank.Tank tankHot(
		...
		use_L = true,
		W_max = W_heater_hot,
		T_set = T_hot_aux_set) annotation(Placement(transformation(extent = {{16, 54}, {36, 74}})));

	// Hot tank 2
	SolarTherm.Models.Storage.Tank.Tank tankHot2(
		redeclare package Medium = Medium,
		D = D_storage,
		H = H_storage,
		T_start = T_hot_start,
		...
		use_L = true,
		W_max = W_heater_hot,
		T_set = T_hot_aux_set) annotation(Placement(transformation(origin={16, 98}, extent = {{0, 0}, {20, 20}})));
	...
	// Pump hot 2
	SolarTherm.Models.Fluid.Pumps.PumpSimple pumpHot2(
		redeclare package Medium = Medium,
		k_loss = k_loss_hot) 
		annotation(Placement(transformation(origin = {72, 101}, extent = {{-6, -6}, {6, 6}})));
	...
	SolarTherm.Models.Storage.Tank.Tank tankCold(
		...
		use_L = true,
		W_max = W_heater_cold,
		T_set = T_cold_aux_set) annotation(Placement(transformation(extent = {{64, -28}, {44, -8}})));

	// Pump cold
	SolarTherm.Models.Fluid.Pumps.PumpSimple pumpCold(
		redeclare package Medium = Medium,
		k_loss = k_loss_cold) annotation(Placement(transformation(extent = {{10, -30}, {-2, -18}})));

	// PowerBlockControl
	SolarTherm.Models.Control.NewPowerBlockControl controlHot(
	...
	SolarTherm.Models.Control.ReceiverControl controlCold(
		...
		Ti = Ti,
		Kp = Kp) annotation(Placement(transformation(extent = {{24, -10}, {10, 4}})));
	...
	SolarTherm.Models.Analysis.Market market(
		...
		annotation(Placement(visible = true, transformation(extent = {{128, 12}, {148, 32}}, rotation = 0)));
...
equation
	...
	// Fluid connections
	connect(pumpCold.fluid_a, tankCold.fluid_b) annotation(Line(points = {{10, -24.12}, {10, -24.12}, {10, -25}, {44, -25}}, color = {0, 127, 255}));
	connect(pumpCold.fluid_b, receiver.fluid_a) annotation(Line(points = {{-24.4, 5.8}, {-14.2, 5.8}, {-14.2, -24}, {-2, -24}}, color = {0, 127, 255}));
	...
	connect(receiver.T, controlCold.T_mea) annotation(Line(points = {{-21, 22}, {30,22}, {30,1}, {26,1}}, color = {0, 0, 127}));
	connect(tankCold.L, controlCold.L_mea) annotation(Line(points = {{24.56, -3}, {38, -3}, {38, -13.6}, {43.8, -13.6}}, color = {0, 0, 127}));
	connect(heliostatsField.on, controlCold.sf_on) annotation(Line(points = {{-72, 2}, {-72, 2}, {-72, -36}, {28, -36}, {28, -6}, {24.7, -6}, {24.7, -7.2}}, color = {255, 0, 255}));
	connect(controlCold.m_flow, pumpCold.m_flow) annotation(Line(points = {{9.16, -3}, {4, -3}, {4, -18.84}}, color = {0, 0, 127}));
...
//	connect(tankHot2.L, controlHot.L_mea_tk2) annotation(Line(points = {{36, 112}, {40, 112}, {40, 65}, {47.52, 65}}, color = {0, 0, 127}));
	...
	connect(sun.solar, heliostatsField.solar) annotation(Line(points = {{-72, 60}, {-72, 36}}, color = {255, 128, 0}));
	connect(heliostatsField.heat, receiver.heat) annotation(Line(points = {{-55.68, 27.5}, {-54.82, 27.5}, {-54.82, 27.4}, {-46, 27.4}}, color = {191, 0, 0}));
	connect(or1.y, heliostatsField.defocus) annotation(Line(points = {{-93.6, 8}, {-92, 8}, {-92, 8.8}, {-87.68, 8.8}}, color = {255, 0, 255}, pattern = LinePattern.Dash));

	//PowerBlock connections
	connect(parasities_input.y, powerBlock.parasities) annotation(Line(points = {{109, 45.7}, {109, 40.85}, {109.6, 40.85}, {109.6, 34.4}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
	connect(powerBlock.W_net, market.W_net) annotation(Line(points = {{115.18, 22.05}, {119.59, 22.05}, {119.59, 22}, {128, 22}}, color = {0, 0, 127}));
	...
	Diagram(
		...
		graphics = {
			Text(lineColor = {217, 67, 180}, extent = {{-96, 92}, {-60, 90}}, textString = "defocus strategy", fontSize = 9),
			Text(lineColor = {217, 67, 180}, extent = {{-50, -40}, {-14, -40}}, textString = "on/off strategy", fontSize = 9),
			Text(origin = {2, 2}, extent = {{-52, 8}, {-4, -12}}, textString = "Receiver", fontSize = 9),
			Text(origin = {12, 4}, extent = {{-110, 4}, {-62, -16}}, textString = "Heliostats Field", fontSize = 9),
			Text(origin = {4, -8}, extent = {{-80, 86}, {-32, 66}}, textString = "Sun", fontSize = 9),
			Text(origin = {20, 50}, extent = {{-10, -5}, {10, 5}}, textString = "Hot Tank", fontSize = 9),
			Text(extent = {{30, -24}, {78, -44}}, textString = "Cold Tank", fontSize = 9),
			Text(origin = {4, -2}, extent = {{80, 12}, {128, -8}}, textString = "Power Block", fontSize = 9),
			Text(origin = {6, 0}, extent = {{112, 16}, {160, -4}}, textString = "Market", fontSize = 9),
			Text(origin = {2, 4}, extent = {{-6, 20}, {42, 0}}, textString = "Rec Control", fontSize = 9),
