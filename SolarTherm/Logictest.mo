...
//Start time
  when y then
    t_on = time;
//t_on=time if on_charge or on_discharge or on_discharge_tk2 becomes true
  end when;
//Stop time
  when not y then
//t_off=time if on_charge or on_discharge or on_discharge_tk2 becomes false
    t_off = time;
  end when;
//Power block starting up
  when t_on - (t_off + t_standby) > 0 then
//startup=true if t_on>(t_off+t_standby) becomes true
    startup = true;
  elsewhen time - t_on > t_start then
//startup=flase if (time-t_on)>t_start becomes true
    startup = false;
  end when;
  standby = time - t_off < t_standby;
//standby=true if (time-t_off)<t_standby at current time step
  annotation(
