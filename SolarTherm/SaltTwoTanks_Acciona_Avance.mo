...
model SaltTwoTanks_Acciona_Avance
  ...
  parameter SI.Diameter D_storage_hot = 50.4 * sqrt(2) "Storage tank diameter";
  ...
  parameter SI.MassFlowRate m_flow_rec_max = m_flow_fac "Maximum mass flow rate to receiver";
  ...
  //parasitic inputs
  Modelica.Blocks.Sources.RealExpression parasities_input(y = heliostat_field_Acciona.W_loss + pumpHot.W_loss + pumpCold.W_loss + tankHot.W_loss) annotation(
  ...
  // Tee
  // Hot tank 1
  SolarTherm.Models.Storage.Tank.Tank tankHot(redeclare package Medium = Medium, D = D_storage_hot, H = H_storage_hot, L_start = (1 - split_cold) * 100, T_set = T_hot_aux_set, T_start = T_hot_start, W_max = W_heater_hot, alpha = alpha, enable_losses = tnk_enable_losses, use_L = true, use_p_top = tnk_use_p_top) annotation(
  ...
  SolarTherm.Tank_Cold_Acciona tank_Cold_Acciona(D = D_storage_cold, H = H_storage_cold, L_start = split_cold * 100, T_set = T_cold_aux_set, T_start = T_cold_start, W_max = W_heater_cold, alpha = alpha, use_L = true) annotation(
  ...
  SolarTherm.Heliostat_field_Acciona heliostat_field_Acciona(A_h = A_heliostat, Q_design = R_des, Wspd_max = Wspd_max, ele_min = ele_min, he_av = he_av_design, lat = data.lat, lon = data.lon, n_h = n_heliostat, nu_defocus = nu_defocus, nu_min = nu_min_sf, nu_start = nu_start, use_defocus = true, use_on = true) annotation(
    Placement(visible = true, transformation(origin = {-67, 21}, extent = {{-13, -13}, {13, 13}}, rotation = 0)));
  SolarTherm.Receiver_Control_Acciona receiver_Control_Acciona(Kp = Kp, L_df_off = cold_tnk_defocus_ub, L_df_on = cold_tnk_defocus_lb, L_off = cold_tnk_crit_lb, L_on = cold_tnk_crit_ub, T_ref = T_hot_set, Ti = Ti, m_flow_max = m_flow_rec_max, y_start = m_flow_rec_start) annotation(
...
//    Line(points = {{36, 68}, {40, 68}, {40, 76}, {47, 76}}, color = {0, 0, 127}));
  ...
  connect(receiver.Recpower, receiver_Control_Acciona.Recpower) annotation(
    Line(points = {{-22, 16}, {24, 16}, {24, 8}, {24, 8}}, color = {0, 0, 127}));
//  connect(receiver.T, receiver_Control_Acciona.T_mea) annotation(
//    Line(points = {{-22, 22}, {38, 22}, {38, 2}, {32, 2}, {32, 2}, {30, 2}}, color = {0, 0, 127}));
  connect(tank_Cold_Acciona.L, receiver_Control_Acciona.level_cold) annotation(
    Line(points = {{56, -18}, {38, -18}, {38, 6}, {32, 6}, {32, 6}, {30, 6}}, color = {0, 0, 127}));
  connect(tankHot.L, receiver_Control_Acciona.level_hot) annotation(
    Line(points = {{36, 68}, {40, 68}, {40, 16}, {12, 16}, {12, 8}, {12, 8}}, color = {0, 0, 127}));
  connect(receiver.heat, heliostat_field_Acciona.heat) annotation(
    Line(points = {{-46, 28}, {-54, 28}, {-54, 28}, {-54, 28}}, color = {191, 0, 0}));
  connect(or1.y, heliostat_field_Acciona.defocus) annotation(
    Line(points = {{-94, 8}, {-88, 8}, {-88, 14}, {-80, 14}, {-80, 14}}, color = {255, 0, 255}));
  connect(Wspd_input.y, heliostat_field_Acciona.Wspd) annotation(
    Line(points = {{-112, 30}, {-96, 30}, {-96, 30}, {-80, 30}, {-80, 30}}, color = {0, 0, 127}));
  connect(sun.solar, heliostat_field_Acciona.solar) annotation(
    Line(points = {{-72, 60}, {-72, 60}, {-72, 48}, {-66, 48}, {-66, 34}, {-66, 34}}, color = {0, 127, 255}));
  connect(tankHot.L, receiver_Control_Acciona.level_hot) annotation(
    Line(points = {{36, 68}, {38, 68}, {38, 24}, {12, 24}, {12, 12}, {12, 12}}, color = {0, 0, 127}));
  connect(receiver_Control_Acciona.level_cold, tank_Cold_Acciona.L) annotation(
    Line(points = {{30, 10}, {38, 10}, {38, -18}, {56, -18}, {56, -18}}, color = {0, 0, 127}));
//  connect(receiver_Control_Acciona.T_mea, receiver.T) annotation(
//    Line(points = {{30, 6}, {36, 6}, {36, 22}, {-24, 22}, {-24, 22}, {-22, 22}}, color = {0, 0, 127}));
  connect(receiver.Recpower, receiver_Control_Acciona.Recpower) annotation(
    Line(points = {{-22, 16}, {-8, 16}, {-8, 30}, {24, 30}, {24, 14}, {24, 14}, {24, 12}}, color = {0, 0, 127}));
  connect(receiver_Control_Acciona.defocus, or1.u2) annotation(
    Line(points = {{20, -10}, {20, -10}, {20, -10}, {-116, -10}, {-116, 4}, {-102, 4}, {-102, 4}}, color = {255, 0, 255}));
  connect(heliostat_field_Acciona.on, receiver_Control_Acciona.sf_on) annotation(
    Line(points = {{-66, 8}, {-68, 8}, {-68, -16}, {34, -16}, {34, -4}, {32, -4}, {32, -4}}, color = {255, 0, 255}));
  connect(receiver_Control_Acciona.m_flow, pumpCold.m_flow) annotation(
    Line(points = {{8, 2}, {4, 2}, {4, -18}, {4, -18}}, color = {0, 0, 127}));
  connect(receiver.fluid_a, pumpCold.fluid_b) annotation(
    Line(points = {{-24, 6}, {-12, 6}, {-12, -24}, {-2, -24}, {-2, -24}}, color = {0, 127, 255}));
  connect(receiver_Control_Acciona.Sun_elev, heliostat_field_Acciona.Sun_elev) annotation(
    Line(points = {{18, 12}, {18, 12}, {18, 20}, {-4, 20}, {-4, -6}, {-50, -6}, {-50, 14}, {-54, 14}, {-54, 14}}, color = {0, 0, 127}));
...
end SaltTwoTanks_Acciona_Avance;